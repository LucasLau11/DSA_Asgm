...
/**
 * Get suggested diagnosis based on visit reason
 */
public String suggestDiagnosisFromVisitReason(String patientId) {
    PatientRecord activeRecord = getActivePatientRecord(patientId);
    if (activeRecord == null) return null;
    
    String visitReason = activeRecord.getVisitReason().toLowerCase();
    
    // Map visit reasons to likely diagnoses
    if (visitReason.contains("fever") || visitReason.contains("temperature")) {
        return "Fever";
    } else if (visitReason.contains("cough") || visitReason.contains("throat")) {
        return "Cough";
    } else if (visitReason.contains("flu") || visitReason.contains("cold")) {
        return "Flu";
    } else if (visitReason.contains("headache") || visitReason.contains("head")) {
        return "Headache";
    } else if (visitReason.contains("stomach") || visitReason.contains("nausea")) {
        return "Stomachache";
    } else if (visitReason.contains("allergy") || visitReason.contains("rash")) {
        return "Allergy";
    } else if (visitReason.contains("checkup") || visitReason.contains("consultation")) {
        return "Other";
    }
...
/**
 * Update visit reason based on final treatment diagnosis
 */
public boolean updateVisitReasonFromDiagnosis(String patientId, String finalDiagnosis) {
    PatientRecord activeRecord = getActivePatientRecord(patientId);
    if (activeRecord == null) return false;
    
    String updatedReason = "Treatment for " + finalDiagnosis;
    activeRecord.setVisitReason(updatedReason);
    return true;
}

/**
 * Remove patient from queue after treatment completion
 */
public boolean removePatientFromQueueAfterTreatment(String patientId) {
    for (int i = 1; i <= patientQueue.getLength(); i++) {
        Patient patient = patientQueue.getEntry(i);
        if (patient.getPatientId().equals(patientId)) {
            patientQueue.remove(i);
            System.out.println("[INFO] Patient " + patientId + " removed from queue after treatment.");
            return true;
        }
    }
    return false;
...
/**
 * Get patient's current visit reason
 */
public String getPatientVisitReason(String patientId) {
    PatientRecord activeRecord = getActivePatientRecord(patientId);
    return activeRecord != null ? activeRecord.getVisitReason() : null;
}

/**
 * Check if patient has pending treatment (in queue)
 */
public boolean isPatientAwaitingTreatment(String patientId) {
    return isPatientInQueue(patientId);
}
    // ========== STATISTICS METHODS ==========
